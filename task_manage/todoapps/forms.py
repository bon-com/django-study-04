...
class TodoFormBase(forms.ModelForm):
    ...
    # タスク必須チェックメッセージ変更
    task = forms.CharField(error_messages={consts.INPUT_REQUIRED: "タスク名は必須です。"})
    # 期日必須チェックメッセージ変更
    due_date = forms.DateTimeField(
        error_messages={consts.INPUT_REQUIRED: "タスク期日は必須です。"}
    )
    # カテゴリ必須チェックメッセージ変更
    ...
    category = forms.ModelChoiceField(
        ...
        error_messages={consts.INPUT_REQUIRED: "カテゴリは必須です。"},
    )
...
class TodoEditForm(TodoFormBase):
    ...
    # ステータス必須チェックメッセージ変更
    status = forms.IntegerField(error_messages={consts.INPUT_REQUIRED: "ステータスは必須です。"})
